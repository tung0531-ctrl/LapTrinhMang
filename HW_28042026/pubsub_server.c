...
#define MAX_TOPICS_PER_CLIENT 32
...
struct client_session {
    ...
    size_t write_length;
    size_t write_sent;
    char topics[MAX_TOPICS_PER_CLIENT][MAX_TOPIC_LENGTH];
    int topic_count;
};
...
static int find_topic_index(const struct client_session *session, const char *topic) {
    int i;

    for (i = 0; i < session->topic_count; ++i) {
        if (strcmp(session->topics[i], topic) == 0) {
            return i;
        }
    }
...
static int subscribe_topic(struct client_session *session, const char *topic) {
    if (find_topic_index(session, topic) >= 0) {
        return 1;
    }

    if (session->topic_count >= MAX_TOPICS_PER_CLIENT) {
        return -1;
    }

    strncpy(session->topics[session->topic_count], topic, MAX_TOPIC_LENGTH - 1);
    session->topics[session->topic_count][MAX_TOPIC_LENGTH - 1] = '\0';
    session->topic_count++;
    return 0;
}

static int unsubscribe_topic(struct client_session *session, const char *topic) {
    int index = find_topic_index(session, topic);

    if (index < 0) {
        return -1;
    }

    for (; index + 1 < session->topic_count; ++index) {
        strcpy(session->topics[index], session->topics[index + 1]);
    }
    session->topic_count--;
    if (session->topic_count >= 0) {
        session->topics[session->topic_count][0] = '\0';
    }
    return 0;
}
...
static void route_message(struct client_session sessions[], const char *topic, const char *message) {
    int i;
    int delivered = 0;
    char payload[READ_BUFFER_SIZE + MAX_TOPIC_LENGTH + 32];

    snprintf(payload, sizeof(payload), "MSG %s %s\n", topic, message);

    for (i = 0; i < MAX_CLIENTS; ++i) {
        if (!sessions[i].active || find_topic_index(&sessions[i], topic) < 0) {
            continue;
        }

        if (append_message(&sessions[i], payload) < 0) {
            fprintf(stderr, "Bo qua client vi bo dem gui da day\n");
            continue;
        }
        delivered++;
    }

    printf("PUB %s -> %d client\n", topic, delivered);
}

static int handle_subscribe(struct client_session *session, const char *topic) {
    char response[160];
    int result;

    if (!is_compact_token(topic) || strlen(topic) >= MAX_TOPIC_LENGTH) {
        return append_message(session, "ERR Topic khong hop le\n");
    }

    result = subscribe_topic(session, topic);
    if (result < 0) {
        return append_message(session, "ERR Da dat toi da so chu de\n");
    }

    if (result > 0) {
        snprintf(response, sizeof(response), "OK Da dang ky %s\n", topic);
        return append_message(session, response);
    }
...
static int handle_unsubscribe(struct client_session *session, const char *topic) {
    char response[160];
    ...
    if (unsubscribe_topic(session, topic) < 0) {
        snprintf(response, sizeof(response), "ERR Chua dang ky %s\n", topic);
        return append_message(session, response);
    }

    snprintf(response, sizeof(response), "OK Da huy dang ky %s\n", topic);
    return append_message(session, response);
}

static int handle_publish(struct client_session sessions[], struct client_session *session, char *arguments) {
    char *topic;
    char *message;
    ...
    topic = strtok(arguments, " \t");
    message = strtok(NULL, "");

    if (topic == NULL || message == NULL) {
        return append_message(session, "ERR Cu phap: PUB <topic> <msg>\n");
    }
    ...
    if (!is_compact_token(topic) || strlen(topic) >= MAX_TOPIC_LENGTH || message[0] == '\0') {
        return append_message(session, "ERR Cu phap: PUB <topic> <msg>\n");
    }

    if (find_topic_index(session, topic) < 0) {
        return append_message(session, "ERR Ban chua dang ky topic nay\n");
    }

    route_message(sessions, topic, message);
    return append_message(session, "OK Da gui tin\n");
}

static int handle_command(struct client_session sessions[], struct client_session *session, char *line) {
    char *command;
    char *arguments;
    ...
    if (strcmp(command, "SUB") == 0) {
        if (arguments == NULL) {
            return append_message(session, "ERR Cu phap: SUB <topic>\n");
        }
        while (*arguments == ' ' || *arguments == '\t') {
            arguments++;
        }
        return handle_subscribe(session, arguments);
    }

    if (strcmp(command, "UNSUB") == 0) {
        if (arguments == NULL) {
            return append_message(session, "ERR Cu phap: UNSUB <topic>\n");
        }
        while (*arguments == ' ' || *arguments == '\t') {
            arguments++;
        }
        return handle_unsubscribe(session, arguments);
    }
    ...
    if (strcmp(command, "PUB") == 0) {
        ...
        return handle_publish(sessions, session, arguments);
    }

    return append_message(session,
                          "ERR Lenh khong hop le. Dung SUB <topic>, UNSUB <topic> hoac PUB <topic> <msg>\n");
}
        ...
        if (received > 0) {
            ...
            while (1) {
...

                {
                    ...
                    if (handle_command(sessions, session, line) < 0) {
                        return -1;
                    }
                }
            }
            ...
            if (session->read_length == sizeof(session->read_buffer) - 1) {
                fprintf(stderr, "Dong lenh qua dai, dong ket noi client\n");
                return -1;
            }
...
int main(void) {
    ...
    printf("Pub/Sub server dang lang nghe cong %d\n", SERVER_PORT);
    printf("Lenh hop le: SUB <topic>, UNSUB <topic>, PUB <topic> <msg>\n");
            ...
            while (1) {
...

                {
                    ...
                    if (append_message(&sessions[slot],
                                       "Chao mung den pub/sub server\n"
                                       "Lenh hop le: SUB <topic>, UNSUB <topic>, PUB <topic> <msg>\n") < 0) {
